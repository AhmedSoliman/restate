...
pub enum Error {
    ...
    #[code(unknown)]
    AdminService(#[from] restate_admin::Error),
...
impl Worker {
    ...
    pub async fn run(self, drain: drain::Watch, admin_service: AdminService) -> Result<(), Error> {
        let (shutdown_signal, shutdown_watch) = drain::channel();

        let worker_command_tx = self.worker_command_tx();
        ...
        let mut admin_handle = tokio::spawn(admin_service.run(
            shutdown_watch.clone(),
            worker_command_tx,
            Some(self.storage_query_context),
        ));
        ...
        tokio::select! {
            ...
            admin_result = &mut admin_handle => {
                admin_result.map_err(|err| Error::component_panic("admin service", err))??;
                panic!("Unexpected termination of admin service.");
            },
