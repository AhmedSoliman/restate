...
/// The set of labels that are allowed to extracted from tracing context to be used in metrics.
/// Be mindful when adding new labels, the number of time series(es) is directly propotional
/// to cardinality of the chosen labels. Avoid using labels with potential high cardinalit
...
pub(crate) fn install_global_prometheus_recorder(opts: &Options) -> PrometheusHandle {
    ...
    metrics::set_global_recorder(recorder).expect("Another global metrics recorder is installed");
    prometheus_handle
}
